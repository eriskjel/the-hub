...
export type OnOpenCallback = (result: {
	caseType: string;
	item: string;
	rarity: string;
}) => void;
...
export function useDrinkCase(
	...
	const handleOpen = () => {
		...
		const chosenIndex = spinStrip.findIndex(
			(d) => d.name === weightedChoice.name,
		);

		const centerOffset = CONTAINER_WIDTH / 2 - ITEM_WIDTH / 2;
		const finalOffset =
			(spinStrip.length * SPIN_ROUNDS + chosenIndex) * ITEM_WIDTH -
			centerOffset;
...
function createWeightedStrip(
	...
	(Object.keys(RARITY_PROBABILITIES) as Array<DrinkVariant["rarity"]>).forEach(
		(rarity) => {
			...
			const allocation = Math.max(
				pool.length,
				Math.round(
					(RARITY_PROBABILITIES[rarity] / 100) * SPINNER_STRIP_BASE_SIZE,
				),
			);
...
function createVisualStrip(
	...
	for (let i = 0; i < rounds; i++) {
		...
		else if (legendaries.length && chosen.rarity === "yellow")
			pool = legendaries;
